					...
					<div>
						<ul className='list-disc list-outside pl-6'>
							{responsibilities.map((el, idx) => (
								<li
									key={idx}
									className='dark:font-extralight my-1 tracking-wide text-gray-600 dark:text-slate-400 group-hover:text-slate-900 group-hover:dark:text-white'
								>
									{el}
								</li>
							))}
				...
				<MotionCardFooter className='gap-2 my-2 flex-wrap'>
					{skills.map(({ Icon, title }, idx) => (
						<MotionChip
							key={idx}
							size='sm'
							className='dark:bg-slate-900 group-hover:dark:bg-slate-950'
						>
